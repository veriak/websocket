...
// AtreugoHandler receives a websocket connection after the handshake has been
// completed. This must be provided.
type AtreugoHandler func(*Conn)

// AtreugoUpgrader specifies parameters for upgrading an HTTP connection to a
// WebSocket connection.
type AtreugoUpgrader struct {
	...
	Error func(ctx *atreugo.RequestCtx, status int, reason error)
	...
	CheckOrigin func(ctx *atreugo.RequestCtx) bool
...
func (u *AtreugoUpgrader) responseError(ctx *atreugo.RequestCtx, status int, reason string) error {
...
func (u *AtreugoUpgrader) selectSubprotocol(ctx *atreugo.RequestCtx) []byte {
...
func (u *AtreugoUpgrader) isCompressionEnable(ctx *atreugo.RequestCtx) bool {
...
// response.
func (u *AtreugoUpgrader) Upgrade(ctx *atreugo.RequestCtx, handler AtreugoHandler) error {
	if !ctx.IsGet() {
	...
	if checkOrigin == nil {
		checkOrigin = atreugocheckSameOrigin
	}
	if !checkOrigin(ctx) {
		return u.responseError(ctx, fasthttp.StatusForbidden, "websocket: request origin not allowed by AtreugoUpgrader.CheckOrigin")
	}
...
// atreugocheckSameOrigin returns true if the origin is not set or is equal to the request host.
func atreugocheckSameOrigin(ctx *atreugo.RequestCtx) bool {
...
// AtreugoIsWebSocketUpgrade returns true if the client requested upgrade to the
// WebSocket protocol.
func AtreugoIsWebSocketUpgrade(ctx *atreugo.RequestCtx) bool {
